...
const checkCookies = (context) => {
    const userName = context.req
        ? Cookies.get('hpbocw_userName', { headers: context.req.headers })
        : Cookies.get('hpbocw_userName');
    const ulb = context.req ? Cookies.get('hpbocw_ulb', { headers: context.req.headers }) : Cookies.get('hpbocw_ulb');
    return { userName, ulb };
};
