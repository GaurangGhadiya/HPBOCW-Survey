...
const Dashboard = () => {
  ...
  const handleFilterChange = ({ district, office }) => {
    ...
    if (district) {
      const queryParams = createQueryParams(district, office);
  ...
  useEffect(() => {
    if (dashboardFilterState?.data) {
      ...
      if (dashboardFilterState?.data) {
        ...
        setSiteList(dashboardFilterState?.data?.siteInfoList);
        setContractorList(dashboardFilterState?.data?.contractorInfoList);


        console.error("setVerificationInfoList", verificationInfoList);
      } else {
        console.error("No data in dashboardFilterState");
      }
    } else {
      console.error("No data in dashboardFilterState");
    }
        ...
        {siteList?.length > 0 && (
          <>
            ...
            <Grid container spacing={1}>
              <Grid item xs={12}>
                <TopCard
                  top_header_data={siteList.map((item) => ({
                    label: item.headerName.toUpperCase(),
                    value: item.headerValueCount ? item.headerValueCount : 0,
                    color: "red", // You can customize this color based on your requirements
                  }))}
        ...
        {contractorList?.length > 0 && (
          <>
            ...
            <Grid container spacing={1}>
              <Grid item xs={12}>
                <TopCard
                  top_header_data={contractorList.map((item) => ({
                    label: item.headerName.toUpperCase(),
                    value: item.headerValueCount ? item.headerValueCount : 0,
                    color: "red", // You can customize this color based on your requirements
                  }))}
